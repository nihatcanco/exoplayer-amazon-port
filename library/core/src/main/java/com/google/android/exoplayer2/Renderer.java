 ...
 *
 * <p>Internally, a renderer's lifecycle is managed by the owning {@link ExoPlayer}. The renderer
 * is
 * transitioned through various states as the overall playback state and enabledA tracks change. The
...
public interface Renderer extends PlayerMessage.Target {
  ...
  /**
   * The renderer is disabledA. A renderer in this state may hold resources that it requires for
   * rendering (e.g. media decoders), for use if it's subsequently enabledA. {@link #reset()} can be
   * called to force the renderer to release these resources.
   */
  int STATE_DISABLED = 0;
  /**
   * The renderer is enabledA but not started. A renderer in this state may render media at the
  ...
  /**
   * Returns the {@link SampleStream} being consumed, or null if the renderer is disabledA.
   */
   ...
   * before it is next disabledA or reset.
   * <p>
   * This method may be called when the renderer is in the following states: {@link #STATE_ENABLED},
   * {@link #STATE_STARTED}.
   */
   ...
   * renderer is next disabledA or reset.
   */
   ...
   * renderer has everything that it needs to continue playback. Returning false indicates that the
   * player should pause until the renderer is ready.
   * <p>
